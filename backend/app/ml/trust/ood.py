...
class FeatureDistributionMonitor:
    ...
    def evaluate(self, feature_vector: dict[str, float] | None) -> dict[str, Any]:
        ...
        if not reference:
            return {"status": "NOT_CONFIGURED", "score": None, "distance": None, "threshold": self.threshold, "per_feature_z": {}, "compared_feature_count": 0, "method": "robust_feature_z_score", "reason": "No authorized reference distribution is configured; OOD detection is not claimed."}
        ...
        if not z_scores:
            return {"status": "NOT_AVAILABLE", "score": None, "distance": None, "threshold": self.threshold, "per_feature_z": {}, "compared_feature_count": 0, "method": "robust_feature_z_score", "reason": "The configured reference shares no features with this image quality vector."}
