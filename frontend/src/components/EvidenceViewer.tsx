import { AlertTriangle, CheckCircle2, Eye, Info, LoaderCircle, SlidersHorizontal } from 'lucide-react';
...
export function EvidenceViewer({ imageId, evidence, loading, error }: EvidenceViewerProps) {
  ...
  const vesselAvailable = vessel?.supported === true && vessel.status === 'model_inference';
  const vesselReliability = !vessel || !vessel.supported ? 'UNAVAILABLE' : vesselAvailable && (vessel.confidence ?? 0) >= 0.75 ? 'HIGH' : 'REVIEW RECOMMENDED';
  ...
  return <div className="card overflow-hidden"><div className="flex flex-col justify-between gap-3 border-b border-line px-5 py-4 sm:flex-row sm:items-center"><div><p className="eyebrow">Clinical evidence</p><h3 className="section-title mt-1 text-base font-extrabold">Evidence viewer</h3></div>{loading ? <StatusBadge tone="teal"><LoaderCircle size={13} className="animate-spin" /> Analyzing structures</StatusBadge> : evidence ? <StatusBadge tone={hasModelEvidence ? 'success' : 'neutral'}>{hasModelEvidence ? 'Model-backed evidence' : 'Experimental / unavailable'}</StatusBadge> : <StatusBadge tone="neutral">Not available</StatusBadge>}</div>
    ...
    <div className="p-5">
      ...
      {evidence && <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{vessel && <div className={`rounded-xl p-3 ${vesselAvailable ? 'bg-emerald-50' : 'bg-amber-50'}`}><p className={`truncate text-[10px] font-bold uppercase tracking-wider ${vesselAvailable ? 'text-emerald-700' : 'text-amber-700'}`}>Retinal vessel analysis</p><p className="mt-2 flex items-center gap-1.5 text-sm font-extrabold text-ink">{vesselAvailable ? <CheckCircle2 size={15} className="text-emerald-600" /> : <AlertTriangle size={15} className="text-amber-600" />}{vesselAvailable ? 'AVAILABLE' : 'UNAVAILABLE'}</p><div className="mt-2 flex items-center justify-between gap-2 text-[10px]"><span className="text-slate-500">Segmentation</span><span className="font-bold text-slate-700">{vesselAvailable ? 'Completed' : 'Not available'}</span></div><div className="mt-1 flex items-center justify-between gap-2 text-[10px]"><span className="text-slate-500">Analysis reliability</span><span className={`font-bold ${vesselReliability === 'HIGH' ? 'text-emerald-700' : 'text-amber-700'}`}>{vesselReliability}</span></div></div>}{lesionModules.filter((module) => module.category === 'lesion_detection').map((module) => <div key={module.module} className="rounded-xl bg-mist p-3"><p className="truncate text-[10px] font-bold uppercase tracking-wider text-slate-400">{module.module.replaceAll('_', ' ')}</p><p className="mt-1 text-sm font-extrabold text-ink">{module.count ?? '—'}</p><p className="mt-1 text-[10px] text-slate-400">{module.status.replaceAll('_', ' ')}</p></div>)}</div>}
    </div>
  </div>;
}
