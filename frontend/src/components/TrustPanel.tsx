...
export function TrustPanel({ result }: { result: TrustResult | null }) {
  ...
  return <div className={`overflow-hidden rounded-2xl border shadow-soft ${cardTone}`}>
    ...
    <div className="px-6 py-6">
      ...
      <div className="mt-6 grid gap-2 sm:grid-cols-2">{FACTORS.map(([key, label]) => { const factor = factorMap.get(key); const available = ['AVAILABLE', 'REAL_MODEL_EVIDENCE', 'COMPLETED'].includes((factor?.status ?? '').toUpperCase()); const flagged = result.risk_flags.some((flag) => (key === 'model_agreement' && flag.code.includes('disagreement')) || (key === 'attention_lesion_agreement' && flag.code.includes('agreement')) || (key === 'ood' && flag.code.includes('shift')) || (key === 'explanation_stability' && flag.code.includes('stability'))); return <div key={key} className="flex items-center gap-3 rounded-xl border border-black/5 bg-white/65 p-3"><span className={flagged ? 'text-amber-600' : available ? 'text-emerald-600' : 'text-slate-400'}>{flagged ? <CircleAlert size={16} /> : available ? <CheckCircle2 size={16} /> : <AlertTriangle size={16} />}</span><span className="min-w-0 flex-1 text-xs font-bold text-ink">{label}</span><span className="text-xs font-extrabold text-ink">{factor?.score == null ? '-' : `${Math.round(factor.score * 100)}%`}</span></div>; })}</div>
