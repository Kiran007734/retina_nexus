...
import { ApiRequestError, createPatient, getPatients, imageContentUrl, runScreening, uploadFundusImage, type ApiErrorCategory, type ScreeningRun } from '../services/api';
...
export function NewScreeningPage() {
  ...
  return <div className="space-y-7">
    <PageIntro eyebrow="Guided workflow" title="Start a new screening" description="Move from trusted image intake to an evidence-linked, self-checking result in one controlled flow." action={<div className="flex flex-wrap gap-2"><Link to="/platform/demo" className="btn-secondary">Explore demo <ArrowRight size={15} /></Link><Link to="/platform/history" className="btn-secondary">View history <ArrowRight size={15} /></Link></div>} />
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className="space-y-5">
        ...
        <div className="flex flex-col-reverse justify-between gap-3 sm:flex-row sm:items-center"><p className="flex items-center gap-2 text-[11px] leading-5 text-slate-400"><Info size={14} className="shrink-0 text-teal-600" /> Ungradable images stop before clinical AI and receive recapture guidance.</p>{run && imageId ? <button onClick={() => navigate('/platform/screening/results', { state: { screeningId: run.screening_id, imageId, run } })} className="btn-primary">Open screening result <ArrowRight size={16} /></button> : <button disabled={!selected || !patientId.trim() || state === 'uploading' || state === 'processing'} onClick={startScreening} className="btn-primary disabled:cursor-not-allowed disabled:opacity-40">Run secure screening <ArrowRight size={16} /></button>}</div>
      </div>
...
function RunSummary({ run, imageId }: { run: ScreeningRun; imageId: string | null }) {
  const quality = run.quality?.final as { quality_decision?: string; quality_band?: string; quality_score?: number; recommended_action?: string; issues?: Array<{ type: string; message: string; recommendation: string }> } | undefined;
  const initialQuality = run.quality?.initial as { quality_score?: number; quality_decision?: string } | undefined;
  const enhancementApplied = Boolean(run.quality?.enhancement_applied);
  const blocked = quality?.quality_decision === 'UNGRADABLE';
  const primaryComplete = ['PRIMARY_RESULT_READY', 'EVIDENCE_PROCESSING', 'FINAL_RESULT_READY', 'COMPLETED'].includes(run.primary_status ?? '') || Boolean(run.classification && run.triage);
  const finalReady = run.status === 'FINAL_RESULT_READY' || run.status === 'COMPLETED';
  return <div className={`rounded-2xl border p-5 ${run.status === 'FAILED' ? 'border-rose-200 bg-rose-50' : blocked ? 'border-amber-200 bg-amber-50' : 'border-teal-100 bg-teal-50/60'}`}><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start"><div><p className="eyebrow">Pipeline result</p><h2 className="section-title mt-1 text-lg font-extrabold">{run.status === 'FAILED' ? 'Run failed safely' : blocked ? 'Recapture recommended' : finalReady ? 'Final screening result ready' : primaryComplete ? 'Primary screening ready' : 'Screening pipeline in progress'}</h2><p className="mt-1 text-xs leading-5 text-slate-600">{run.message}</p></div><StatusBadge tone={run.status === 'FAILED' ? 'danger' : blocked ? 'warning' : finalReady ? 'success' : 'teal'}>{run.status}</StatusBadge></div>{run.error && <p className="mt-4 rounded-xl bg-white/70 p-3 text-xs leading-5 text-rose-800">Stage <strong>{run.error.stage}</strong>: {run.error.message}</p>}{quality && <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Mini label="Quality band" value={`${quality.quality_band ?? quality.quality_decision ?? '—'} · ${Math.round((quality.quality_score ?? 0) * 100)}%`} /><Mini label="Image" value={imageId ? 'Stored securely' : 'Unavailable'} /><Mini label="Evidence" value={run.evidence_status === 'AVAILABLE' ? 'Available' : run.evidence_status === 'PROCESSING' ? 'Processing' : run.evidence_status ?? 'Not run'} /><Mini label="Next action" value={blocked ? 'Recapture image' : run.triage?.recommendation ?? 'Review result'} /></div>}{enhancementApplied && imageId && <div className="mt-4 rounded-xl border border-teal-100 bg-white/70 p-4"><div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-center"><div><p className="text-xs font-extrabold text-ink">Controlled enhancement and recheck</p><p className="mt-1 text-[11px] leading-5 text-slate-600">{initialQuality?.quality_decision === 'BORDERLINE' ? 'Borderline image detected → one controlled enhancement pass applied.' : 'One controlled enhancement pass was applied.'} Post-enhancement result: <strong>{quality?.quality_decision ?? 'Unavailable'}</strong>.</p></div><span className="rounded-lg bg-teal-50 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wider text-teal-800">AI preprocessing / quality enhancement</span></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><figure><img src={imageContentUrl(imageId, 'original')} alt="Original fundus image" className="h-36 w-full rounded-lg bg-slate-900 object-contain" /><figcaption className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">Original</figcaption></figure><figure><img src={imageContentUrl(imageId, 'enhanced')} alt="Enhanced fundus image" className="h-36 w-full rounded-lg bg-slate-900 object-contain" /><figcaption className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-400">Enhanced · preprocessing only</figcaption></figure></div></div>}{blocked && quality?.issues && quality.issues.length > 0 && <div className="mt-4 space-y-2">{quality.issues.map((issue) => <div key={issue.type} className="rounded-xl bg-white/70 p-3 text-xs"><p className="font-extrabold text-ink">{issue.type.replaceAll('_', ' ')}</p><p className="mt-1 text-slate-600">{issue.message}</p><p className="mt-1 font-semibold text-teal-800">{issue.recommendation}</p></div>)}</div>}</div>;
}
