...
import { createPatient, getPatients, runScreening, uploadFundusImage, type ScreeningRun } from '../services/api';
...
export function NewScreeningPage() {
  ...
  async function startScreening() {
    if (!selected || !patientId.trim()) return;
    setError(''); setRun(null); setState('uploading');
    try {
      let patient: { id: string };
      try { patient = await createPatient({ anonymized_identifier: patientId.trim(), age_group: 'not recorded' }); } catch (createError) {
        const existing = await getPatients(); const match = existing.find((item) => item.anonymized_identifier === patientId.trim()); if (!match) throw createError; patient = match;
      }
    ...
    } catch (requestError) { setState('error'); setError(requestError instanceof Error ? requestError.message : 'The screening pipeline could not process this image.'); }
  }
  ...
  return <div className="space-y-7">
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_330px]">
      <div className="space-y-5">
        ...
        {state === 'error' && <div role="alert" className="rounded-2xl border border-rose-200 bg-rose-50 p-5 text-sm text-rose-800"><div className="flex items-start gap-3"><AlertTriangle size={18} className="mt-0.5 shrink-0" /><div><p className="font-extrabold">Screening could not be completed</p><p className="mt-1 leading-6">{error}</p><p className="mt-2 text-xs text-rose-700">No clinical prediction has been created. Check the API and registered model configuration before retrying.</p></div></div></div>}
