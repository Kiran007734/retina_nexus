import { AlertTriangle, ArrowLeft, Download, FileText, Info, LoaderCircle, Sparkles } from 'lucide-react';
...
function EvidenceSummary({ run }: { run: ScreeningRun }) { const modules = Object.values(run.lesions?.modules ?? {}); return <div className="surface p-5"><p className="eyebrow">Clinical evidence</p><h3 className="section-title mt-1 text-base font-extrabold">Supporting findings</h3><div className="mt-4 space-y-2">{modules.length === 0 ? <p className="text-xs leading-5 text-slate-500">No lesion module output was available for this run.</p> : modules.map((module) => <div key={module.module} className="flex items-center justify-between gap-3 rounded-xl bg-mist p-3"><span className="truncate text-xs font-bold capitalize text-ink">{module.module.replaceAll('_', ' ')}</span><span className="text-xs font-extrabold text-slate-600">{module.count ?? module.status}</span></div>)}</div></div>; }
